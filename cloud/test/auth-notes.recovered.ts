...
beforeAll(async () => {
  await env.DB.exec(`
    ...
    CREATE TABLE webauthn_credentials (id TEXT PRIMARY KEY, user_id TEXT NOT NULL, public_key TEXT NOT NULL, counter INTEGER NOT NULL DEFAULT 0, transports_json TEXT NOT NULL DEFAULT '[]', device_type TEXT NOT NULL, backed_up INTEGER NOT NULL DEFAULT 0, rp_id TEXT NOT NULL, created_at TEXT NOT NULL, updated_at TEXT NOT NULL);
    CREATE TABLE webauthn_challenges (id TEXT PRIMARY KEY, kind TEXT NOT NULL, user_id TEXT, challenge TEXT NOT NULL, rp_id TEXT NOT NULL, origin TEXT NOT NULL, expires_at TEXT NOT NULL, created_at TEXT NOT NULL);
...
describe('SHIKITYPE auth and notes', () => {
  ...
  it('issues a user-verified discoverable-passkey registration ceremony only for a logged-in account', async () => {
    const account = await signup('passkey-register');
    const blocked = await call('/api/auth/passkey/register/options', { method: 'POST' });
    expect(blocked.status).toBe(401);
    const response = await call('/api/auth/passkey/register/options', { method: 'POST' }, account.cookie);
    expect(response.status).toBe(200);
    const body = await response.json<{ challengeId: string; options: { rp: { id: string; name: string }; user: { name: string }; challenge: string; authenticatorSelection: { residentKey: string; userVerification: string } } }>();
    expect(body.challengeId).toMatch(/^wch_[a-f0-9]{48}$/);
    expect(body.options.rp).toEqual({ id: 'shikitype.example', name: 'SHIKITYPE' });
    expect(body.options.user.name).toBe(account.id);
    expect(body.options.challenge.length).toBeGreaterThan(20);
    expect(body.options.authenticatorSelection).toEqual(expect.objectContaining({ residentKey: 'required', userVerification: 'required' }));
    const stored = await env.DB.prepare('SELECT user_id, kind, rp_id, origin FROM webauthn_challenges WHERE id = ?').bind(body.challengeId).first<{ user_id: string; kind: string; rp_id: string; origin: string }>();
    expect(stored).toEqual({ user_id: expect.any(String), kind: 'registration', rp_id: 'shikitype.example', origin });
  });

  it('uses discoverable credentials for login without receiving a login ID', async () => {
    const response = await call('/api/auth/passkey/login/options', { method: 'POST', body: '{}' });
    expect(response.status).toBe(200);
    const body = await response.json<{ challengeId: string; options: { rpId: string; challenge: string; allowCredentials?: unknown; userVerification: string } }>();
    expect(body.challengeId).toMatch(/^wch_[a-f0-9]{48}$/);
    expect(body.options.rpId).toBe('shikitype.example');
    expect(body.options.challenge.length).toBeGreaterThan(20);
    expect(body.options.allowCredentials).toBeUndefined();
    expect(body.options.userVerification).toBe('required');
  });

  it('consumes a passkey challenge after a failed verification so it cannot be replayed', async () => {
    const account = await signup('passkey-replay');
    const options = await (await call('/api/auth/passkey/register/options', { method: 'POST' }, account.cookie)).json<{ challengeId: string }>();
    const invalid = { challengeId: options.challengeId, credential: { id: 'not-a-real-credential', rawId: 'not-a-real-credential', type: 'public-key', response: {} } };
    expect((await call('/api/auth/passkey/register/verify', { method: 'POST', body: JSON.stringify(invalid) }, account.cookie)).status).toBe(400);
    expect((await call('/api/auth/passkey/register/verify', { method: 'POST', body: JSON.stringify(invalid) }, account.cookie)).status).toBe(400);
    expect(await env.DB.prepare('SELECT id FROM webauthn_challenges WHERE id = ?').bind(options.challengeId).first()).toBeNull();
  });

  it('rejects an unknown passkey assertion without creating a session', async () => {
    const options = await (await call('/api/auth/passkey/login/options', { method: 'POST', body: '{}' })).json<{ challengeId: string }>();
    const response = await call('/api/auth/passkey/login/verify', {
      method: 'POST',
      body: JSON.stringify({ challengeId: options.challengeId, credential: { id: 'abcdefgh', rawId: 'abcdefgh', type: 'public-key', response: { userHandle: 'dXNlci1oYW5kbGU' } } }),
    });
