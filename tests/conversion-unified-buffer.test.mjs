...
await reset('symbol'); await typeRaw('p'); state = await conversion(); ok('p一打は小文字pを先頭、対応するπを2番目にする', state.candidates[0] === 'latin-lower-p' && state.candidates[1] === 'pi', state); await page.keyboard.press('Enter'); ok('p → Enterでπではなく小文字pを確定する', await value() === 'p', await value());
await reset('symbol'); await typeRaw('a'); state = await conversion(); ok('a一打も小文字aの次にαを置く', state.candidates[0] === 'latin-lower-a' && state.candidates[1] === 'greek-alpha', state);
...
await reset('symbol'); await typeRaw('pai'); const beforeSpace = await conversion(); await page.keyboard.press('Space'); state = await conversion(); ok('物理Spaceは候補を確定・取消しせず細い数式空白を入れる', state.raw === 'pai' && JSON.stringify(state.candidates) === JSON.stringify(beforeSpace.candidates) && (await value()).includes('\\,'), { beforeSpace, state, latex: await value() }); await page.keyboard.press('Enter'); ok('Space後もEnterで候補を確定できる', (await value()).includes('\\pi'), await value());
await reset('symbol'); await typeRaw('pai'); const beforeScreenSpace = await conversion(); await page.click('[data-special="Space"]'); state = await conversion(); ok('画面Spaceも候補を確定・取消しせず細い数式空白を入れる', state.raw === 'pai' && JSON.stringify(state.candidates) === JSON.stringify(beforeScreenSpace.candidates) && (await value()).includes('\\,'), { beforeScreenSpace, state, latex: await value() }); await page.keyboard.press('Enter'); ok('画面Space後もEnterで候補を確定できる', (await value()).includes('\\pi'), await value());
...
await reset('symbol'); await page.click('[data-special="Text"]'); await page.keyboard.type('hello'); await page.keyboard.press('Space'); await page.keyboard.type('world'); const textLatexWithSpace = await page.evaluate(() => window.__neoApp.getActiveRow().mf.value); ok('文内physical SpaceはOS標準の文章空白として残る', textLatexWithSpace.includes('hello world'), textLatexWithSpace); await page.keyboard.press('Enter');
await reset('symbol'); await page.click('[data-special="Text"]'); await page.keyboard.type('screen'); await page.click('[data-special="Space"]'); await page.keyboard.type('space'); const textLatexWithScreenSpace = await page.evaluate(() => window.__neoApp.getActiveRow().mf.value); ok('文内画面Spaceも通常の文章空白として残る', textLatexWithScreenSpace.includes('screen space'), textLatexWithScreenSpace); await page.keyboard.press('Enter');
